...
Comme le \emph{système} \LaTeX{} est constitué de logiciels libres, je présente d’abord ce que j’entends par logiciel libre et en quoi importe l’utilisation de logiciels libres et publics au lieu de logiciels privés.
...
\item Le système \TeX{} peut être installé grâce à ce qu’on appelle une \emph{distribution}. Les principales distributions sont gérées par des organismes à but non lucratif et sont disponibles sur le Web.
\end{itemize}
\end{defex}
...
\begin{enumerate}
\item Grâce à une \emph{adresse} (une URL), on localise un \emph{document} qui se trouve sur un ordinateur (un serveur) quelque part dans le monde;
...
J’ai une nouvelle réjouissante pour tous: depuis quelque temps, le catalogue Ariane de notre chère bibliothèque rend disponibles les données du catalogue au format Bib\TeX{}! Je dis bien Bib\TeX{}, donc le format de l’ancien système (sûrement à cause des préférences des gens en sciences de la nature, qui ont beaucoup influencé l’adoption de \LaTeX{} par la \entite{FESP}), mais les deux formats sont largement compatibles. Il suffit souvent d’ajouter quelques entrées pour rendre les données d’Ariane encore plus complètes.
...
Par exemple, dans le présent document, la plupart des italiques que vous voyez ont été appliqués \emph{parce que} l’intention de l’auteur est d’accentuer les propos mis en italique. L’aspect visuel correspond donc dans ce cas à une intention rhétorique.
...
\begin{defex}
Utiliser un système comme \LaTeX{}, c’est donc simplement écrire soi-même son balisage au lieu de laisser un logiciel l’ajouter automatiquement. Les deux approches ont leurs avantages, mais un langage comme celui de \LaTeX{} permet entre autres les choses suivantes:
...
Tandis qu’on parle des logiciels de mise en page, je vous livre une nouvelle en \emph{primeur}:
...
En comparaison, la formule de la rédaction et de l’édition avec \LaTeX{} est simple et \emph{convaincante}:
